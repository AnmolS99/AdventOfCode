...
def can_equation_be_true(answer, values):
    ...
    if len(values) == 2:
        return can_equation_be_true(answer, [left_elems_sum]) or can_equation_be_true(answer, [left_elems_product])
    
    return can_equation_be_true(answer, [left_elems_sum] + values[2:]) or can_equation_be_true(answer, [left_elems_product] + values[2:])
